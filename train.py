...
自己记录：
本篇代码多地方使用了assert语句，assert语句能够让系统直接抛出异常，当assert后面跟随的表达式不正确的时候，assert后面的语句执行，否则跳过继续执行
"""
import matplotlib
...
def train(hyp, opt, device):
    ...
    # Configure
    # 是否需要画图: 所有的labels信息、前三次迭代的batch、训练结果等
    ...
    if RANK in [-1, 0]:
        ...
        # W&B  wandb日志打印相关
        opt.hyp = hyp  # add hyperparameters添加超参数
        # 迁移学习的部分，加载别人训练好的模型，并从断点开始训练
    ...
    if pretrained:
        ...
        # 模型是RGB三通道的输入，关于改为npy文件输入目前有两种思路：（1）将灰度图转化为RGB（2）将三通道改为单通道，但不知道效果如何
        ...
        if RANK in [-1, 0]:
            ...
            if not notest or final_epoch:  # Calculate mAP
                ...
                results, maps, _ = val.run(data_dict,  # 数据集配置文件地址 包含数据集的路径、类别个数、类名、下载地址等信息
                                           ...
                                           save_dir=save_dir,  # 保存地址 runs/train/exp
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default='data/TAO2.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--image-weights', default=True, action='store_true',
...
def main(opt):
    ...
    if opt.resume and not wandb_run:
        ...
        # 如果本身传递的就是一维的2俩个数组[a,a]那么就直接是原值，如果不是则进行复制一个出来
