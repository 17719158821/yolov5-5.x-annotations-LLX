            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                # 如果路径path为包含图片路径的txt文件
                    ...
                    with open(p, 'r') as t:
                        ...
                        parent = str(p.parent.parent.parent) + os.sep + str('images')+ os.sep
                        # f += [x.replace('./', parent) if x.startswith('./') else x for x in t]  # local to global path
                        f += [x.replace('./', parent) if x.startswith('./') else (parent+x) for x in t]  # local to global path
                        getpatientindex=[]
                        for patientpath in f:
                            for index in os.listdir(patientpath):
                                #在此筛选出图片是否有对应的标签，做两种方案，一种标签全部喂进去，一种不带标签的提出
                                if os.path.exists(os.path.join(patientpath.replace('images','labels'),index.split('.')[0]+'.txt')):
                                    getpatientindex.append(os.path.join(patientpath, index))
                        f=getpatientindex
                        # f = glob.glob(str(f[0] / '**' / '*.*'), recursive=True)
